...
ARM_EMBEDDED_TOOLCHAIN_VERSION="4.9.3"
...
if [ ! -d "$ARM_EMBEDDED_CXX_HEADERS/arm-linux-gnueabihf" ]; then
  echo "ERROR: ARM_EMBEDDED_CXX_HEADERS is not valid"
  [ "$0" = "$BASH_SOURCE" ] && exit 1 || return 1
fi
...
if [ -z "$ARM_EMBEDDED_FLAGS" ]; then
  export ARM_EMBEDDED_FLAGS="-I$ARM_EMBEDDED_SYSROOT/sysroot/usr/include -I$ARM_EMBEDDED_CXX_HEADERS -I$ARM_EMBEDDED_CXX_HEADERS/arm-linux-gnueabihf -L$ARM_EMBEDDED_SYSROOT/sysroot/usr/lib"
fi
